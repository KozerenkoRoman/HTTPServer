{****************************************************************************************}
{                                                                                        }
{Приклад xml-файлу з UsedAttribute = [uaTimeChange, uaValue]:                            }
...
{</Root>                                                                                 }
{                                                                                        }
{Приклад xml-файлу з UsedAttribute = []:                                                 }
{<?xml version="1.0" encoding="windows-1251"?>                                           }
{<Root>                                                                                  }
{  <Cash_register>                                                                       }
...
{</Root>                                                                                 }
{                                                                                        }
{ Робота з довільними атрибутами                                                         }
{   //Встановити атрибути і їх значення                                                  }
{   xmlFile.Attributes.Node := xmlFile.GetNode(xmlFile.GetXPath('SECTION', 'Key'));      }
{   або для CurrentSection <> ''                                                         }
...
{   //Записати встановлені значення в файл                                               }
{   xmlFile.WriteAttributes;                                                             }
{                                                                                        }
{                                                                                        }
{   //Зчитати значення з файлу                                                           }
{   if xmlFile.ReadAttributes('SomeSection', 'SomeKey') then                             }
{   begin                                                                                }
{     //Зчитати значення в відповідні змінні                                             }
...
{   end;                                                                                 }
{                                                                                        }
{  //Зчитати значення з секції, в якій всі назви елементів не унікальні                  }
{  //і відповідають C_KEY                                                                }
...
type
  ...
  private
    FActive             : Boolean;
    FAttributes         : TXMLAttributes;     // список атрибутів, що будуть автоматично використовуватися в поточному елементі
    FBackup             : Boolean;
  ...
  public
    ...
    function IsLastKey: Boolean;                                                                   // вказує, що поточний ключ останній в поточній секції
    procedure NextKey;                                                                             // переходить до наступного ключа в поточній секції
    ...
    procedure ReadToFile(const aSection, aKey, aFileName: string);                                 // зберігає значення xml-файлу в зовнішній файл
    ...
    property Backup         : Boolean            read FBackup             write FBackup;           // ознака, чи потрібно архівувати попередню копію файла перед записом
    property CurrentSection : string             read FNameCurrentSection write SetCurrentSection; // Визначає поточну секцію
    property EncodeStream   : Boolean            read FEncodeStream       write FEncodeStream;     // ознака, чи потрібно кодувати/розкодовувати поток перед/після запису до файла в формат Base64
...
begin
  //перевірка, чи файл вже не сконвертовано
  aItems := nil;
  aIniFile := nil;
